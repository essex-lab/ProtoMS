    ...
    def dup(self, par):
        for i in self.pars:
            if (
                par.type == i.type
                and par.chg == i.chg
                and par.sigma == i.sigma
                and par.epsilon == i.epsilon
            ):
                return True, i
...
class AmbParameterSet:
    ...
    def read_in(self, filename, term=None):
        ...
        if term is None or term == "center":
            ...
            raise Exception(
                "Invalid value given as term argument. Must be None, center, cterm or nterm"
            )
        with open(filename) as f:
            for line in f:
                ...
                if len(cols) == 11:
                    ...
                    self.cljs.add(
                        self.atms[at2], float(cols[10]), name, at_type, res
                    )
...
scl14lj    0.500\n"""
        ...
        for i, j in enumerate(sorted(self.dihs), self.start):
            ...
            terms = tuple(
                range(
                    (i - self.start) * 5 + self.start,
                    ((i - (self.start - 1)) * 5 + self.start),
                )
            )
        ...
        with open(os.path.join(home, "parameter", "amber99-residues.ff")) as f:
            for line in f:
                if line.startswith("residue"):
                    ...
                    s += " ".join(cols[:3])
                    s += " {0} {0}\n".format(
                        self.cljs.cljbyres(res, at).id + (self.start - 1)
                    )
                    ...
                    s += " ".join(cols[:2])
                    s += " {0} {0} ".format(
                        self.cljs.cljbyres(res, conv["backbone"][at]).id
                        + (self.start - 1)
                    )
                    s += " ".join(cols[4:]) + "\n"
            ...
            for line in f:
                if line.startswith("residue"):
                    ...
                    at = cols[1]
                    s += " ".join(cols[:2])
                    s += " {0} {0} {1} {1} {2} {2} ".format(
                        self.cljs.cljbyres(res, conv[res][at]).id
                        + (self.start - 1),
...
class PmsResidue:
    def __init__(self):
        self.chains = {}
        self.sidechains = {"nterm": {}, "cterm": {}, "center": {}}
        self.rest = []
            ...
            for line in f:
                if line.startswith("residue"):
                    ...
                    if mode == "chain":
                        ...
                        res.add_atom(
                            cols[1], int(cols[2]), int(cols[4]), int(cols[6])
                        )

                elif line.startswith(
                    ("zmat", "bond", "angle", "dihedral", "info", "backbone")
                ):
                    if mode == "chain":
