...
format_strings = {"a": "%{0}s", "E": "%{0}e", "I": "%{0}d"}
data_type = {"a": str, "E": float, "I": int}
...
class prmtop:
    ...
    def get_angle_by_at(self, at1, at2, at3):
        for i in chunks(
            self["ANGLES_INC_HYDROGEN"].vals
            + self["ANGLES_WITHOUT_HYDROGEN"].vals,
            4,
        ):
            if sorted([at1, at2, at3]) == sorted(
                [ptoi(int(j)) for j in i[:3]]
            ):
                parm_set = int(i[3])
                return (
                    self["ANGLE_FORCE_CONSTANT"].vals[parm_set - 1],
                    self["ANGLE_EQUIL_VALUE"].vals[parm_set - 1],
                )
        else:
            raise MissingParamError(
                "Unable to find angle associated with atoms %d %d %d"
                % (at1, at2, at3)
            )
