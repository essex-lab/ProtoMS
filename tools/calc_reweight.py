...
"""
Note this program is extremely experimental and its
...
def replica_histogram(energy="total", replica="temperature", variable="theta",
                      which='1', ewidth=100.0, rwidth=1.0, results=["results"],
                      skip=0, plothist=True):
    """
    ...
    # This might be the most general way, but I am not sure if this is
    # the best one, because I could directly store thetas in terms
    # of temperatures. Same for temperatures.
        ...
        for snap in results_file.snapshots:
            if energy == "total":
                ...
                raise simulationobjects.SetupError(
                    "This tipe of energy is not supported.")

    # I also need to know which theta I want among all those that are there!
    # (for the different solutes)!!!!!!
            ...
            if variable == "theta":
                ...
                raise simulationobjects.SetupError(
                    "This tipe of variable is not supported.")

            if replica == "temperature":
                arrreplicas.append(snap.temperature)
            else:
                raise simulationobjects.SetupError(
                    "This tipe of replica is not supported.")
    ...
    if not rwidth:
        try:
            ...
            raise simulationobjects.SetupError(
                "Automatic replica bin width could not be stated. Please, provide an argument for --replicabin.")

    arr_rbin_edges = np.array([replica-(rwidth/2)
                               for replica in replicas] + [replicas[-1]+(rwidth/2)])
    arr_ebin_edges = np.array(
        [np.amin(arrenergies)+ewidth*i for i in range(nebins+1)])

    hist_enre = np.histogram2d(arrenergies, arrreplicas, bins=[
                               arr_ebin_edges, arr_rbin_edges])
...
def set_variables(hist, replica, main):
    ...
    if isinstance(main, str):
        ...
        try:
            mybeta = float(main)
            if replica == "temperature":
                mybeta = 1/(kb*(float(main)+C2K))
            logger.info("Replica %s chosen as main." % main)
        except:
            mybeta = beta[0][0]
            logger.info(
                "Replica %s could not be interpreted. Lowest replica chosen as main." % main)
...
def twham(hist, replica="temperature", tolerance=0.001, gcorr=1.0, iterations=500, out_freq=1000, main=None):
    ...
    histval, energy, beta, mybeta, ewidth, rwidth = set_variables(
        hist, replica, main)
    ...
    for iteration in range(iterations):
        ...
        density_est = np.expand_dims(np.sum(
            histval/gcorr, axis=1)/np.sum(Nk*ewidth*np.exp(fe-beta*energy)/gcorr, axis=1), 1)

        fe_new = np.expand_dims(
            (-1)*np.log(np.sum(density_est*ewidth*np.exp((-1)*beta*energy), axis=0)), 0)
        ...
        if (np.absolute(fe-fe_new) < tolerance).all():
            density_est = np.expand_dims(np.sum(
                histval/gcorr, axis=1)/np.sum(Nk*ewidth*np.exp(fe_new-beta*energy)/gcorr, axis=1), 1)
            break
...
def weight(hist, replica="temperature", main=None):
    ...
    if main is None:
        main = hist[2][0] - rwidth/2

    if not isinstance(main, float):
        main = float(main)

    myreplica = [ind for ind, leftedge in enumerate(
        np.delete(hist[2], -1)) if leftedge < main < hist[2][ind+1]][0]
...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(
        description="Program to reweight energies from different replicas with WHAM.")
    parser.add_argument('-f', '--files', nargs="+",
                        help="the path of the files to analyse. Default is results. ", default=["results"])
    parser.add_argument('-r', '--replicas', choices=[
                        "temperature"], help="which replicas we are using", default="temperature")
    parser.add_argument('-v', '--variable', choices=[
                        "theta", "totalenergy"], help="the variable to reweight", default="theta")
    parser.add_argument('-w', '--weight', choices=["single", "twham"],
                        help="estimate of the density of states (weighting factor). 'single' uses mainreplica, 'twham' uses all replicas. Default=single", default="single")
    parser.add_argument(
        '-s', '--skip', help="number of snapshots to skip from the results file. Default=0.", default='0')
    parser.add_argument(
        '-eb', '--energybin', help="the width of the bins for the energy", default=100.0)
    parser.add_argument('-rb', '--replicabin',
                        help="the width of the bins for the temperature", default=None)
    parser.add_argument(
        '-wv', '--whichvar', help="if more than one instance of the variable is in files, which of them is to be reweighted. Default=1", default=1)
    parser.add_argument(
        '--iterations', help="the number of WHAM iterations allowed. Default=500", default=500)
    parser.add_argument(
        '--mainreplica', help="the replica of interest. By default, the lowest one is chosen.", default=None)
    parser.add_argument('--plothistogram', action="store_true",
                        help="whether to plot a histogram of the variable counts per energy and replica bin. Default=false", default=False)
    ...
    try:
        ...
        raise simulationobjects.SetupError(
            "Expected integer after flag '-s' / '--skip'.")

    try:
        iterations = int(args.iterations)
    except:
        raise simulationobjects.SetupError(
            "Value %s for --iterations could not be interpreted as integer" % args.iterations)
    ...
    if args.replicabin != None:
        try:
            ...
            raise simulationobjects.SetupError(
                "Value %s for --replicabin could not be interpreted as float" % args.replicabin)

    hist_enre, varmeans = replica_histogram(replica=args.replicas, variable=args.variable, which=args.whichvar, ewidth=float(
        args.energybin), rwidth=args.replicabin, results=args.files, skip=skip)

    if args.weight == "twham":
        weight, energy, beta, mybeta = twham(
            hist_enre, replica=args.replicas, iterations=iterations, main=args.mainreplica)
    elif args.weight == "single":
        weight, energy, beta, mybeta = weight(
            hist_enre, replica=args.replicas, main=args.mainreplica)
    else:
        raise simulationobjects.SetupError(
            "Value %s for --weight could not be interpreted." % args.weight)

    result = weighted_variable(
        varmeans, weight, args.mainreplica, energy, beta, mybeta, args.replicas)
