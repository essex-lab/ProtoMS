...
def _make_vdw_tem(tem1,tem2,pdb1,pdb2,cmap,usepdb=True) :
  ...
  def find_param(atoms,temset,gaffset) :
    ...
    for param in gaffset.params.itervalues() :
  ...
  for ptype in ["bond","angle","dihedral"] :
    gaffsets[ptype] = sim.ParameterSet(ptype,gaffname)
  ...
  for atom in atomlist :
    ...
    make_variable(atom)
    # Then add variable geometry for all other atoms bonded to this atom, except if it is in the atomlist
    for i,atom2 in enumerate(tem1.templates[0].atoms) :
      if variablemade[i] or atom2 in atomlist : continue
