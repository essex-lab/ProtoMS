...
# ----------------------------------------------------------------
    ...
    """
        if cent[0:4] == "cent":
            ...
            if len(cols) == 3:  # Gives the center
                ...
                # Calculate the center of geometry of atoms between
                # cols[0] and cols[1]
                first = int(cols[0]) - 1
                last = int(cols[1])
    ...
    logger.debug(
        "This will solvate either a protein or a ligand "
        "using a pre-equilibrated box"
    )

    # Check for presence of required inputs and change pdb files
    # into pdb objects
    if ligand is None and protein is None:
        raise simulationobjects.SetupError(
            "Neither ligand nor protein have been provided - quitting!"
        )
    ...
    try:
        for i in range(len(solvent["xyz"])):
            if len(solvent["xyz"][i]) == len(added_water["xyz"][0]):
                ...
                raise simulationobjects.SetupError(
                    "The length of a crystal water molecule does not match the"
                    " length of molecules in the water box - please check "
                    "they are the same type (TIP3P/TIP4P)!"
                )
    except KeyError:
        pass
    ...
    for i, w in enumerate(added_water["xyz"]):
        ...
        if len(w) > 3:
            newatom4 = simulationobjects.Atom(
                index=atmidx,
                name=names[3],
                resindex=resid,
                resname=resname[len(w) - 1],
