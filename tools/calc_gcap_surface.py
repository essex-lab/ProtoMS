...
class SurfaceCalculation(fe.FreeEnergyCalculation):
    ...
    def _body(self, args):
        ...
        for est in self.estimators:
            fep_pmfs = np.array([result[est].pmf.as_floats()
                                 for result in fep_results]).T

            surface = np.zeros(len(self.lambdas[0][0]) * len(self.B_values))
            surf, fopt = optimize.fmin_bfgs(
                _obj_func, surface, args=(gci_pmfs, fep_pmfs),
                disp=False, full_output=True)[:2]

            surf = surf.reshape(fep_pmfs.shape)
            fig = plot_surface(lams, Bs, surf, 'dA (kcal/mol)',
                               'Free Energy Surface - %s' % est.__name__)
            self.figures['pmf2d_%s' % est.__name__] = fig
            results[est] = surf
        fig = plot_surface(lams, Bs,
                           np.array([result.occupancies.as_floats()
                                     for result in gci_results]),
                           'Number of Waters', 'Water Occupancy Surface')
