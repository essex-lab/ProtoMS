...
def GetMolTemplate(FileName, MolName):
...
def printpdb(coords,molpdb,resid,theta,filename):
...
if __name__ == "__main__":
  ...
  filenum = 0
  while len(molpdb) == 0:
    molpdb = GetMolTemplate(args.files[filenum], args.molecule)
    filenum += 1
  ...
  for i in occ_order[::-1]:
    printpdb(clusts[i],molpdb,resid,occupancies[i],outfile)
    resid += 1
