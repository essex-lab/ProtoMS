...
parser = argparse.ArgumentParser ( description = 'Script to modify ProtoMS simulation output '
...
parser.add_argument ( '-i', '--in_file', required = True, help = 'pdb trajectory input file' )
parser.add_argument ( '-o', '--out_file', required = True, help = 'pdb trajectory output file' )
parser.add_argument ( '-n', '--nwat', required = True, help = 'Largest number of inserted '
                                             ' GC waters in input trajectory file', type = int )
args = parser.parse_args()


def make_ref ( index ):
    """Create a new reference water whenever one is missing from the GCMC trajectory.
    """
