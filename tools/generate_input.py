...
class GCAPDual(ProteinLigandSimulation) :
  """ 
  This a command file for a dual-topology protein-ligand GCAP simulation
  Generates input for proteins, solutes and solvent
  write dual-toplogy parameters, lambda-replica exchange,
  dump results, pdb and restart files
  equilibrate and production run
  ...
  def __init__(self,protein="protein.pdb",
    ...
    if isinstance(adamval,float) or isinstance(adamval,int):
      ...
      self.setParameter("multigcmc","%d %s " % (dumpfreq," ".join("%.3f"%a for a in adamval) ))
...
class GCAPSingle(ProteinLigandSimulation) :
  ...
  def __init__(self,protein="protein.pdb",
    ...
    """      
    if len(outfolder) == 0 : outfolder = "out_gcap"          
    ProteinLigandSimulation.__init__(self,protein=protein,solutes=solutes,solvent=solvent,templates=templates,outfolder=outfolder,ranseed=ranseed)
...
def generate_input(protein,ligands,templates,protein_water,ligand_water,ranseed,settings) :
  ...
  if settings.simulation == "equilibration" :
    ...
    # cmdcls = {"gcap_single":GCAPSingle,"gcap_dual":GCAPDual}
    ...
    if protein is not None :
      bnd_cmd = cmdcls_bnd[settings.simulation](protein=protein,solutes=ligands, 
                             gcmcbox=settings.gcmcbox,gcmcwater=settings.gcmcwater,adamval=settings.adams,
                             templates=templates,solvent=protein_water,watmodel=settings.watmodel,
                             lambdaval=lambdavals,nequil=settings.nequil,ranseed=ranseed,
                             nprod=settings.nprod,dumpfreq=settings.dumpfreq,outfolder=outfolder+"_gcap",restrained=rest_solutes)
...
if __name__ == "__main__":
  ...
  if bnd_cmd is not None : 
    if args.simulation == "gcmc" :
      ...
      bnd_cmd.writeCommandFile(args.out+"_gcap.cmd") 
