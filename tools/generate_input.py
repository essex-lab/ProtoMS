...
def _assignMoveProbabilities(protein, solute, solvent, moveset, isperiodic):
    ...
    # The amount by which the protein be accelerated over the solvent.
    accel_prot = 5
    # The amount by which the solute be accelerated over the solvent.
    accel_solu = 50
    # Move proportion add up to this number. This is just for clarity.
    addto = 1000.0
...
class ProteinLigandSimulation(ProtoMSSimulation):
    ...
    def __init__(self, protein="protein.pdb",
        ...
        self.setForceField(os.path.join(
            "$PROTOMSHOME", "parameter", "amber14SB.ff"))
        self.setForceField(os.path.join(
            "$PROTOMSHOME", "parameter", "solvents.ff"))
        self.setForceField(os.path.join(
            "$PROTOMSHOME", "parameter", "amber14SB-residues.ff"))
        self.setForceField(os.path.join(
            "$PROTOMSHOME", "parameter", ProteinLigandSimulation.gaffversion + ".ff"))
...
class Equilibration(ProteinLigandSimulation):
    ...
    def __init__(self, protein="protein.pdb",
        ...
        """
        ProteinLigandSimulation.__init__(self, protein=protein, solutes=solutes,
                                         solvent=solvent, templates=templates, outfolder=outfolder, ranseed=ranseed)
...
class DualTopology(ProteinLigandSimulation):
    ...
    def __init__(self, protein="protein.pdb",
        ...
        self.setParameter("softcoreparams",
                          "coul 1 delta 0.2 deltacoul 2.0 power 6 soft66")
        self.setParameter("dlambda", "0.001")
        self.setParameter("lambdare", "%d %s" % (
            2 * dumpfreq, " ".join("%.3f" % l for l in lambdaval)))
        ...
        if protein is not None:
            for restsol in restrained:
                ...
                for ind, tem in enumerate(templates):
                    ...
                    for mol_template in temobj.templates:
                        if mol_template.name in pdbobj.header:
                            ...
                            resatom = str(mol_template.atoms[
                                          0]).strip().split()
                ...
                self.setChunk("restraint add %d cartesian harmonic %.3f %.3f %.3f %d" % (
                    restsol + 1, atmcoords[0], atmcoords[1], atmcoords[2], FORCE_CONSTANT))
...
class SingleTopology(ProteinLigandSimulation):
    ...
    def __init__(self, protein="protein.pdb",
        ...
        ProteinLigandSimulation.__init__(
            self, protein=protein, solutes=solutes, solvent=solvent,
            templates=templates, outfolder=outfolder, ranseed=ranseed)
...
class RestraintRelease(ProteinLigandSimulation):
    ...
    def __init__(self, protein="protein.pdb",
        ...
        for restsol in restrained:
            ...
            self.setChunk("id add %d solute %d %s %s" %
                          (restsol + 1, restsol + 1, resatom[1], resname))
            self.setChunk("restraint add %d cartesian harmonic %.3f %.3f %.3f %d lambda" % (
                restsol + 1, atmcoords[0], atmcoords[1], atmcoords[2], FORCE_CONSTANT))
...
class GCMC(ProteinLigandSimulation):
    ...
    def __init__(self, protein="protein.pdb",
        ...
        if watmodel == "tip4p":
            self.setForceField(os.path.join(
                "$PROTOMSHOME", "data", "gcmc_tip4p.tem"))
        elif watmodel == "tip3p":
            self.setForceField(os.path.join(
                "$PROTOMSHOME", "data", "gcmc_tip3p.tem"))
        ...
        if isinstance(adamval, float) or isinstance(adamval, int):
            ...
            self.setParameter("multigcmc", "%d %s " % (
                2 * dumpfreq, " ".join("%.3f" % a for a in adamval)))
        ...
        self.setChunk(
            "equilibrate %d solvent=0 protein=0 solute=0 insertion=333 deletion=333 gcsolute=333" % nequil)
        self.setChunk("equilibrate %d %s" % (nequil, moves))
        self.setChunk("simulate %d %s" % (nprod, moves))
...
class Jaws2(ProteinLigandSimulation):
    ...
    def __init__(self, protein="protein.pdb",
        ...
        if isinstance(jbias, float) or isinstance(jbias, int):
            ...
            self.setParameter("multijaws2", " ".join(
                "%.3f" % j for j in jbias))
        if watmodel == "tip4p":
            self.setForceField(os.path.join(
                "$PROTOMSHOME", "data", "gcmc_tip4p.tem"))
...
def generate_input(protein, ligands, templates, protein_water, ligand_water,
    ...
    if settings.simulation == 'sampling':
        ...
        if ligands is None:
            raise sim.SetupError(
                "No ligands loaded, cannot do free energy simulations.")

        cmdcls = {"dualtopology": DualTopology,
        ...
        cmd_kwargs = {'lambdaval': lambdavals, 'nequil': settings.nequil,
                      'ranseed': ranseed, 'nprod': settings.nprod,
        ...
        # if settings.simulation == "singletopology":
        #     command_files['gas'] = cmdcls[settings.simulation](
        #         protein=None, solutes=ligands[:2],
        #         templates=templates, solvent=None,
        #         outfolder=outfolder + "_gas", restrained=rest_solutes,
        #         **cmd_kwargs)
        # elif (settings.simulation == "dualtopology" and settings.absolute
        #       and protein is not None):
        #     command_files['restr'] = RestraintRelease(
        #         protein=protein, solutes=ligands[:1], templates=templates[:1],
        #         solvent=protein_water, lambdaval=(0., 1.),
        #         nequil=settings.nequil, ranseed=ranseed, nprod=settings.nprod,
        #         dumpfreq=settings.dumpfreq, outfolder=outfolder + "_bnd_rstr",
        #         restrained=rest_solutes[:1])

        # command_files['free'] = cmdcls[settings.simulation](
        #     protein=None, solutes=ligands[:2], templates=templates,
        #     solvent=ligand_water, outfolder=outfolder + "_free",
        #     restrained=rest_solutes, **cmd_kwargs)

        # if protein is not None:
        #     command_files['bnd'] = cmdcls[settings.simulation](
        #         protein=protein, solutes=ligands, templates=templates,
        #         solvent=protein_water, outfolder=outfolder + "_bnd",
        #         restrained=rest_solutes, **cmd_kwargs)
        ...
        if settings.adamsrange is not None:
            if len(settings.adamsrange) not in (2, 3):
                msg = "If using --adamsrange, specify exactly two Adams " \
                      "values and optionally the number of desired values"
                logger.error(msg)
        ...
        command_files['jaws'] = Jaws1(
            protein=protein, solutes=ligands, templates=templates,
            solvent=protein_water, jawswater=settings.gcmcwater,
...
def get_arg_parser():
    ...
    parser.add_argument(
        '-s', '--simulation', choices=["sampling", "equilibration",
                                       "dualtopology", "singletopology",
                                       "gcmc", "jaws1", "jaws2"],
        help="the kind of simulation to setup", default="equilibration")
    parser.add_argument(
        '--dovacuum', action='store_true',
        help="turn on vacuum simulation for simulation types"
             " equilibration and sampling", default=False)
    parser.add_argument(
        '-p', '--protein', help="the name of the protein file")
    parser.add_argument(
    ...
    parser.add_argument(
        '-pw', '--protwater',
        help="the name of the solvent for protein")
    parser.add_argument(
        '-lw', '--ligwater',
        help="the name of the solvent for ligand")
    parser.add_argument(
        '-o', '--out',
        help="the prefix of the name of the command file", default="run")
    parser.add_argument(
        '--outfolder', help="the ProtoMS output folder", default="out")
    parser.add_argument(
        '--gaff',
        help="the version of GAFF to use for ligand", default="gaff16")
    parser.add_argument(
        '--lambdas', nargs="+", type=float,
        help="the lambda values or the number of lambdas", default=[16])
    parser.add_argument(
        '--adams', nargs="+", type=float,
        help="the Adam/B values for the GCMC", default=0)
    parser.add_argument(
        '--adamsrange', nargs="+", type=float,
        ...
        default=None)
    parser.add_argument(
        '--jawsbias', nargs="+", type=float,
        help="the bias for the JAWS-2", default=0)
    parser.add_argument(
    ...
    parser.add_argument(
        '--dumpfreq', type=float, help="output dump frequency", default=1E5)
    parser.add_argument(
    ...
    parser.add_argument(
        '--ranseed',
        help="the value of the random seed you wish to simulate with. If None,"
             " then a seed is randomly generated. Default=None", default=None)
    parser.add_argument(
...
if __name__ == "__main__":
    ...
    for cmd in command_files:
        command_files[cmd].writeCommandFile("%s_%s.cmd" % (args.out, cmd))
