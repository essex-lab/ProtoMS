...
def calc_rmsd(ref,structures,resname,atomname=None) :
  ...
  if atomname is not None : atomname = atomname.lower()

  centers = []
  for pdb in structures.pdbs :
    for i,res in pdb.residues.iteritems() :
      if res.name.lower() != resname : continue
      center = np.zeros(3)
      for atom in res.atoms : 
        if atomname is not None and atom.name.strip().lower() != atomname : continue
...
def get_arg_parser():
  ...
  parser = argparse.ArgumentParser(description="Program to calculate RMSD of ligand centre")
  parser.add_argument('-i','--initial',help="the initial PDB-file of the ligand")
...
if __name__ == "__main__":
  ...
  print "(to use this as an harmonic restraint in ProtoMS, specify %.3f)"%(k*0.5)
