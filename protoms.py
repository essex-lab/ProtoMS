...
def _wizard(settings):
    ...
    if instr in ["jon", "dev"]:
        ...
        img = np.load(simulationobjects.standard_filename(".ee.npz", "tools"))[
            instr
        ]
        plt.imshow(img)
        plt.show()
    ...
    print(
        "\tPlease enter the protein name or a PDB filename.\n\tPress enter"
    ...
    if settings.simulation not in [
        ...
        while instr == "":
            print(
                "For the type of simulation you have selected, two ligands "
    ...
    print(
        "\tPress enter if you don't have a solute or when you have "
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-c", "--cmdfile", default="run", help="the prefix of the command file"
    )
    parser.add_argument(
    ...
    liggroup.add_argument(
        "--charge", nargs="+", type=float, help="the net charge of each ligand"
    )
    liggroup.add_argument(
    ...
    simgroup.add_argument(
        "--dumpfreq", type=float, default=1e5, help="the output dump frequency"
    )
    simgroup.add_argument(
        "--ranseed",
        default=None,
    ...
    if os.getenv("PROTOMSHOME") is None:
        ...
        os.environ["PROTOMSHOME"] = (
            string  # This does not change the original shell
        )
    ...
    if not os.path.isfile(args.waterbox):
        msg = (
            "Could not find file (%s) with pre-equilibrated waters"
            % args.waterbox
        )
        logger.error(msg)
    ...
    if args.ligand is not None:
        ...
        for _ligand in args.ligand:
            ...
            ligand_files[prefix]["pdb"], ligand_files[prefix]["obj"] = (
                _load_ligand_pdb(prefix, args.folders)
            )
        ...
        if args.simulation == "dualtopology" and len(ligands) < 2:
            if args.absolute:
                ...
                prefix0 = ligands[0]
                dummy_name = (
                    os.path.basename(_get_prefix(prefix0)) + "_dummy.pdb"
                )
                ligand_files["*dummy"] = {}
                ...
                ligand_files["*dummy"]["tem"] = (
                    simulationobjects.standard_filename("dummy.tem", "data")
                )
                logger.info("")
                logger.info(
                    "Creating dummy PDB-file for ligand: %s"
                    % ligand_files["*dummy"]["pdb"]
                )
            else:
                msg = (
                    "You are trying to run a dual topology simulation "
    ...
    for repeat in repeats:
        ...
        if (
            args.simulation not in ["singletopology", "gcap_single", "jaws2"]
            or "_ele" in repeat
        ):
            ...
            args.simulation in ["singletopology", "gcap_single"]
            and "_vdw" in repeat
        ):
            free_cmd, bnd_cmd, gas_cmd = generate_input(
                protein_file,
