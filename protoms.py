...
logger = sim.setup_logger("protoms_py.log")


class SystemComponent(object):
    def __init__(self, pdb_file_name, folders=[]):
        ...
        if self.pdb.name == "":
            msg = "Structure file %s.pdb could not be found" % self.name
            logger.error(msg)
            raise sim.SetupError(msg)


class Ligand(SystemComponent):
    def __init__(self, pdb_file_name, charge, folders=[]):
        SystemComponent.__init__(self, pdb_file_name, folders)

        self.charge = charge
        self.residue_name = self.pdb.residues[1].name
        self.template = None
        self.template_file_path = None

    def prepare(self, settings):
        """Searches for and loads a ProtoMS template and water box for a
        ligand. If these don't exist perform the necessary steps to
        create them. The settings argument is an argparse Namespace
        that specifies required information."""
        logger.info('Preparing ligand %s.' %
        ...
        if template_file_path is None:
            ...
            # first look for necessary intermediate files
            prepi_file_path = _locate_file(self.name + '.prepi',
                                           settings.folders)
            frcmod_file_path = _locate_file(self.name + '.frcmod',
                                            settings.folders)
            zmat_file_path = _locate_file(self.name + '.zmat',
                                          settings.folders)

            # if prepi is missing run antechamber to create it
            if prepi_file_path is None:
                tools.run_antechamber(
                    self.pdb_file_path, self.charge, self.residue_name)
                prepi_file_path = _locate_file(self.name + '.prepi',
                                               settings.folders)

            # if frcmod is missing or the prepi was, remake the frcmod
            if prepi_file_path is None or frcmod_file_path is None:
                tools.run_parmchk(self.pdb_file_path)
                frcmod_file_path = _locate_file(self.name + '.frcmod',
                                                settings.folders)
            ...
            # if a new zmat was created by tools.build_template then save it
            if zmat_file_path is None:
...
class Protein(SystemComponent):
    def prepare(self, ligands, settings):

        if settings.scoop is None:
            # A pre-prepared protein was not provided
            # so have to prepare manually
            if ("_scoop" in settings.protein) or ("_pms" in settings.protein):
                logger.info(
                    "The protein file %s may have been prepared already. "
                    "This may cause file duplications and confusing naming "
                    "conventions. Consider using the --scoop flag instead of "
                    "the --protein flag to avoid this." % settings.protein)

            # rename atoms and residues for ProtoMS conventions
            renamed_pdb = _rename_protein(self.pdb, settings)
            # Convert existing waters to chosen water model
            renamed_pdb = tools.convertwater(
                renamed_pdb, settings.watmodel)

            scooped_pdb = tools.scoop(
                renamed_pdb, _get_scoop_center(renamed_pdb, ligands, settings),
            ...
            # test if scoop was actually performed, update structure
            # and filename accordingly
            if len(scooped_pdb.residues) == len(self.pdb.residues):
                self.pdb_file_path = self.name + '_pms.pdb'
                logger.info(
                    "Not enough residues removed by scoop. Created %s "
                    "instead. This structure has had atoms and "
                    "residues renamed according to ProtoMS conventions."
                    % self.pdb_file_path)
            else:
                self.pdb = scooped_pdb
                self.pdb_file_path = self.name + '_scoop.pdb'
                logger.info(
                    "Created scooped structure - %s." % self.pdb_file_path)
        ...
        if water_path is not None:
            ...
            self.water_pdb = tools.solvate(
                ...
                protein=self.pdb, geometry='droplet', padding=10.0,
...
def _get_scoop_center(protein_pdb, ligands, settings):
    # Defining the center of the scoop...
    if settings.ligand is None and settings.gcmcbox is None:
        if settings.center is not None:
            center = settings.center
        else:
            protein_pdb.getCenter()
            center = "%f %f %f" % tuple(protein_pdb.center)
            logger.warning("Warning: No specified center for protein scoop. "
                           "Using the center of the protein.")
    elif settings.ligand is None and settings.gcmcbox is not None:
        center = sim.PDBFile(filename=settings.gcmcbox[0])
        logger.info("Scooping protein around %s..." % settings.gcmcbox[0])
    else:
        center = sim.merge_pdbs(lig.pdb for lig in ligands)
    return center


def _rename_protein(protein, settings):
    # Trying to locate a default conversions file
    if settings.atomnames is None:
        conversionfile = sim.standard_filename(
    ...
    renamed_pdb = tools.pdb2pms(protein, "amber", conversionfile)
    return renamed_pdb
...
def _merge_templates(templates, tarlist):
    ...
    if isinstance(temfile, sim.TemplateFile):
        ...
        logger.info(
            "Created a re-numbered template files for all ligands: %s" % templates[0])
        return templates
    else:
        return [temfile]
...
def _load_ligand_pdb(ligprefix, folders):
    ...
    return pdbfile, sim.PDBFile(filename=pdbfile)
...
def _prep_ligand(files, first, charge, ligobj12, folders, tarlist, settings):
    ...
    if len(files["obj"].residues) < 1:
        if len(files["obj"].solvents) > 0:
            raise sim.SetupError(
                "Your ligand in %s is recognized as solvent. Please change the residue name." % files["obj"].name)
        else:
            raise sim.SetupError(
    ...
    if 'HEADER' in files["obj"].header:
        ...
        logger.info(
            'Unable to find header information in the PDB-file, adding it automatically.')
    ...
    if settings.template is None:
        ...
        for f in settings.template:
            ...
            if tempfile is None:
                ...
                tem = sim.TemplateFile(filename=tempfile)
    ...
    if files["tem"] is None:
        ...
        if files["prepi"] is None:
            ...
            files["prepi"] = tools.run_antechamber(
                files["pdb"], charge, resnam)
        ...
        # By this stage we should have all necessary files to make the template
        # file
        ...
        if files["zmat"] is None:
            ...
            logger.info(
                "Created zmatrix (%s) for ligand. Please check the output carefully" % files["zmat"])
            # tarlist.append(files["zmat"])
        logger.info(
            "Created ProtoMS template-file (%s) for ligand. Please check the output carefully" % files["tem"])
...
def _prep_protein(protprefix, ligands, watprefix, folders, tarlist, settings):
    ...
    if protein_orig_file is None and protein_scoop_file is None and protein_pms_file is None:
        msg = "Protein file (%s.pdb) and protein scoop file (%s.pdb) and protein pms file (%s_pms.pdb) could not be found" % (
    ...
    if protein_scoop_file is None and protein_pms_file is None:
        ...
        if ligands is None and settings.gcmcbox is None:
            if settings.center != None:
                ...
                logger.warning(
                    "Warning: No specified center for protein scoop. Using the center of the protein.")
        elif ligands is None and settings.gcmcbox is not None:
            ligobj = sim.PDBFile(filename=settings.gcmcbox[0])
            logger.info("Scooping protein around %s..." % settings.gcmcbox[0])
        else:
            ligobj = ligands
        ...
        if nresdiff < settings.scooplimit:
            if not abortedconv:
                ...
                protobj.write(filename=protein_pms_file,
                              header=header, solvents=False)
            else:
...
def _prep_singletopology(pdbs, templates1, tarlist, settings):
    ...
    eletem, vdwtem, combtem, cmap = tools.make_single(
        tem1, tem2, pdbs[0], pdbs[1], settings.singlemap, gaffversion=settings.gaff)
    ...
    prefix = os.path.join(os.path.dirname(pdbs[0]), tem1.templates[
                          0].name.lower() + "t" + tem2.templates[0].name.lower())
    ...
    logger.info(
        "Created template %s for van der Waals-perturbation. Please check the output carefully." % templates2[0])
    logger.info(
        "Created template %s for combined perturbation. Please check the output carefully." % templates3[0])
...
def _prep_gcmc(ligands, ligand_files, waters, tarlist, settings):
    ...
    def pdb2box(pdbobj, padding=2.0):
        ...
        logger.info("")
        logger.info(
            "Created %s to visualize GCMC/JAWS-1 simulation box. Please check the output carefully" % boxpdb)
        return boxpdb

    def arrange_wats(box, water_file, out_name):
        # find smallest box including all oxygens in my water_file
        waterobj = sim.PDBFile(filename=water_file)
        if len(waterobj.residues) != 0:
            waterbox = waterobj.getBox(
                atomlist=[waterobj.residues.itervalues().next().atoms[0].name])
        else:
            waterbox = waterobj.getBox(
                atomlist=[waterobj.solvents.itervalues().next().atoms[0].name])

        # check whether the box of the water oxygens is within the box provided
        # as argument
        ...
        if not (box_origen_below and box_end_avobe) or (np.all(waterbox['len'] < 0.5) and len(waterobj.solvents) > 1):
            ...
            # make sure the header in the waterobj is the box dimensions which
            # will be used in this simulation
            box_extremes = tuple(box['origin']) + \
                tuple(np.add(box['origin'], box['len']))
    ...
    def fill_box(settings, boxpdb, box, ghost_name):
        ...
        if settings.gcmcwater is None:
            # Creating ghost waters with a density 1.5 times the density of
            # bulk water.
            box_volume = box['len'][0] * box['len'][1] * box['len'][2]
            # 0.0334 waters per Angs.^3 is the number density of bulk water.
            ghostnum = str(int(np.ceil(box_volume * 0.0334 * 1.5)))
            ghostobj = tools.distribute_particles(
...
#        for atom in ghostobj.solvents[sol].atoms : atom.resname = "WA1"
    ...
    if settings.gcmcbox is not None and len(settings.gcmcbox) is 1:
        ...
        if gcmcbox is None:
            msg = "File %s given as gcmcbox could not be found." % settings.gcmcbox[
                0]
            logger.error(msg)
            raise sim.SetupError(msg)
        settings.gcmcbox = gcmcbox
    elif settings.gcmcbox is not None and len(settings.gcmcbox) < 6:
        msg = "6 arguments expected to define the GCMC/JAWS1 box dimensions, %d provided: %s" % (
    ...
    if settings.gcmcbox is not None and isinstance(settings.gcmcbox, str):
        gcmcboxobj = sim.PDBFile(filename=settings.gcmcbox)
        # Try to find the box in the header of the file
        box = sim.find_box(gcmcboxobj)
        if box is None:
            boxpdb = pdb2box(gcmcboxobj, padding=0.0)
            gcmcboxobj = sim.PDBFile(filename=boxpdb)
            box = sim.find_box(gcmcboxobj)
        else:
            boxpdb = settings.gcmcbox
        if "center" in box:
            box['origin'] = np.array(
                [coord - box["len"][ind] / 2 for ind, coord in enumerate(box["center"])])
    ...
    # If the dimensions of the gcmc box have been provided
        ...
        try:
            box = {}
            box['origin'] = np.array([float(value)
                                      for value in settings.gcmcbox[:3]])
            box['len'] = np.array([float(value)
                                   for value in settings.gcmcbox[3:]])
        except:
            raise sim.SetupError(
                "The box dimensions %s could not be correctly interpreted" % settings.gcmcbox)
        sim.write_box(boxpdb, box)
        logger.info("")
        logger.info(
            "Created %s to visualize GCMC/JAWS-1 simulation box. Please check the output carefully" % boxpdb)
        # Fill the box with waters
    ...
    # If no box information has been provided but we have a gcmcwater file
        ...
        waterobj = sim.PDBFile(filename=settings.gcmcwater)
        box = sim.find_box(waterobj)
        if box is None:
            raise sim.SetupError(
    ...
    if write:
        ...
        logger.info("")
        logger.info(
            "Created %s; it contains the GCMC or JAWS-1 simulation waters. Please check the output carefully" % ghost_name)
...
def _prep_jaws2(water_file, tarlist, settings):
    ...
    logger.info("")
    logger.info(
        "Creating water PDB-files for JAWS-2 called jaws2_wat*.pdb and jaws2_not*.pdb")
    single_wat.write(["jaws2_wat%d.pdb" % (i + 1)
                      for i in range(len(single_wat.pdbs))])
    other_wat.write(["jaws2_not%d.pdb" % (i + 1)
                     for i in range(len(single_wat.pdbs))])
...
#     subprocess.call("tar -cf prep_files.tar %s" %
#                     " ".join(tarlist2), shell=True)
...
def _wizard(settings):
    ...
    if instr in ["jon", "dev"]:
        ...
        img = np.load(sim.standard_filename(
            ".ee.npz", "tools"))[instr]
        plt.imshow(img)
        plt.show()
        return
    ...
    if instr == "":
        return
    val = int(instr)
    vals = ["equilibration", "sampling", "dualtopology",
...
def get_arg_parser():
    ...
    parser.add_argument('-l', '--ligand', nargs="+", default=[],
                        help="the prefix of the ligand(s)")
    parser.add_argument(
        '-w', '--water', help="the prefix of the water/solvent", default="water")
    parser.add_argument('-c', '--cmdfile',
                        help="the prefix of the command file", default="run")
    parser.add_argument(
        '-sc', '--scoop', help="the name of your protein scoop")
    parser.add_argument('-t', '--template', nargs="+",
                        help="the template files for your ligands")
    parser.add_argument(
        '-r', '--repeats', help="the number of repeats to be run (if more than 1) or a name for your repeat", default="")
    ...
    cntrlgroup.add_argument(
        '--setupseed', help="optional seed for random number generators in setup", default=None, type=int)
    # Ligand setup variables
    liggroup = parser.add_argument_group("Ligand setup variables")
    liggroup.add_argument('--charge', nargs="+", type=float, default=[],
    ...
    protgroup.add_argument(
        '--center', help="the center of the scoop, if ligand is not available, either a string or a file with the coordinates", default=None)
    protgroup.add_argument('--innercut', type=float,
                           help="maximum distance from ligand defining inner region of the scoop", default=16.0)
    protgroup.add_argument('--outercut', type=float,
                           help="maximum distance from ligand defining outer region of the scoop", default=20.0)
    protgroup.add_argument('--flexin', choices=['sidechain', 'flexible', 'rigid'],
                           help="the flexibility of the inner region", default="flexible")
    protgroup.add_argument('--flexout', choices=['sidechain', 'flexible', 'rigid'],
                           help="the flexibility of the outer region", default="sidechain")
    protgroup.add_argument(
        '--scooplimit', help="the minimum difference between number of residues in protein and scoop for scoop to be retained", default=10)
    protgroup.add_argument('--capradius', type=float,
                           help="the radius of the droplet around the protein", default=30.0)
    # Simulation parameters
    simgroup = parser.add_argument_group("Simulatiom parameters")
    simgroup.add_argument('--lambdas', nargs="+", type=float,
                          help="the lambda values or the number of lambdas", default=[16])
    simgroup.add_argument('--adams', nargs="+", type=float,
                          help="the Adam/B values for the GCMC", default=0)
    simgroup.add_argument('--adamsrange', nargs="+", type=float,
                          help="the upper and lower Adam/B values for the GCMC and, optionally, the number of values desired (default value every 1.0), e.g. -1 -16 gives all integers between and including -1 and -16", default=None)
    simgroup.add_argument(
        '--gcmcwater', help="a pdb file with a box of water to do GCMC on or an integer corresponding to the number of water molecules to add")
    simgroup.add_argument('--gcmcbox', nargs="+",
                          help="a pdb file with box dimensions for the GCMC box, or a list of origin(x,y,z) and length(x,y,z) coordinates")
    simgroup.add_argument('--jawsbias', type=float, nargs="+",
                          help="the bias in JAWS-2", default=[6.5])
    simgroup.add_argument('--nequil', type=float,
                          help="the number of equilibration steps", default=5E6)
    simgroup.add_argument('--nprod', type=float,
                          help="the number of production steps", default=40E6)
    simgroup.add_argument('--dumpfreq', type=float,
                          help="the output dump frequency", default=1E5)
    simgroup.add_argument(
        '--ranseed', help="the value of the random seed you wish to simulate with. If None, then a seed is randomly generated. Default=None", default=None)
    simgroup.add_argument('--absolute', action='store_true',
                          help="whether an absolute free energy calculation is to be run. Default=False", default=False)
    simgroup.add_argument('--dovacuum', action='store_true',
                          help="turn on vacuum simulation for simulation types equilibration and sampling", default=False)
    simgroup.add_argument('--testrun', action='store_true',
                          help="setup a short test run. Default=False", default=False)
    simgroup.add_argument('--cleanup', action='store_true',
                          help="Clean up extra files. Default=False", default=False)
    simgroup.add_argument('--tune', action='store_true',
                          help='Carry out dihedral tuning simulation', default=False)
    return parser
...
if __name__ == "__main__":
    ...
    logger.debug("Running protoms.py at %s" %
    ...
    if os.getenv("PROTOMSHOME") is None:
        ...
        # This does not change the original shell
        os.environ["PROTOMSHOME"] = string

    # Try to find a default water box for solvating
    if args.waterbox is None:
        args.waterbox = sim.standard_filename(
    ...
    # load and prepare all ligand structures
    if len(args.ligand) != len(args.charge):
        raise sim.SetupError(
            "Exactly one charge must be specified for each ligand.")
    ligands = [Ligand(ligname, c)
               for ligname, c in zip(args.ligand, args.charge)]
    for lig in ligands:
        lig.prepare(args)
    ...
    #     gcmcbox_pdb = sim.PDBFile(args.gcmcbox[0])
    #     foci += (gcmcbox_pdb,)
    ...
    #             ligand_files[
    #                 "*dummy"]["obj"] = tools.make_dummy(ligand_files[prefix0]["obj"])
    #             ligand_files[
    #                 "*dummy"]["obj"].write(ligand_files["*dummy"]["pdb"])
    #             ligand_files[
    #                 "*dummy"]["tem"] = sim.standard_filename("dummy.tem", "data")
    #             logger.info("")
    #             logger.info("Creating dummy PDB-file for ligand: %s" %
    #                         ligand_files["*dummy"]["pdb"])
    #         else:
    #             msg = "You are trying to run a dual topology simulation but with only one ligand! (%s.pdb)\nIf you meant to run an absolute free energy calculation, rerun with the --absolute option." % ligands[
    #                 0]
    #             logger.error(msg)
    #             raise sim.SetupError(msg)
    ...
    #             ligobj12 = None
    #         prefix = _get_prefix(l)
    #         _prep_ligand(ligand_files[prefix], i == 0,
    #                      charge, ligobj12, args.folders, tarlist, args)
    ...
    #         logger.error(msg)
    #         raise sim.SetupError(msg)
    #     args.gcmcwater, water_file = _prep_gcmc(
    #         ligands, ligand_files, water_file, tarlist, args)
    ...
    #         free_cmd, bnd_cmd, gas_cmd = tools.generate_input(
    #             protein_file, ligpdbs, ligtems, water_file, ligand_water, ranseed, args)
    #     elif args.simulation == "singletopology" and "_vdw" in repeat:
    #         free_cmd, bnd_cmd, gas_cmd = tools.generate_input(
    #             protein_file, ligpdbs, ligtems2, water_file, ligand_water, ranseed, args)
    #     elif args.simulation == "singletopology" and "_comb" in repeat:
    #         free_cmd, bnd_cmd, gas_cmd = tools.generate_input(
    #             protein_file, ligpdbs, ligtems3, water_file, ligand_water, ranseed, args)
    #     elif args.simulation == "jaws2":
    #         idx = int(repeat.split("-")[-1][1:])
    #         args.gcmcwater = "jaws2_wat%d.pdb" % idx
    #         jaws2wat = "jaws2_not%d.pdb" % idx
    #         free_cmd, bnd_cmd, gas_cmd = tools.generate_input(
    #             protein_file, ligpdbs, ligtems, water_file + " " + jaws2wat, ligand_water, ranseed, args)
    ...
    #             gas_cmd.writeCommandFile(
    #                 args.cmdfile + repeat + "_bnd_rstr.cmd")
    #         else:
