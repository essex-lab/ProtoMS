...
def _prep_ligand(files,first,charge,ligobj12,folders,tarlist,settings) :
  ...
  if files["wat"] is None :
    ...
    if (settings.simulation in ["dualtopology","singletopology"] and not first) or (settings.simulation in ["gcmc","gcap_single","gcap_dual","jaws1","jaws2"]) :
...
def _prep_gcmc(ligands,ligand_files,waters,tarlist,settings) :
  ...
  def pdb2box(pdbobj,padding=2.0) :
    if settings.simulation in ["gcap_single","gcap_dual"]:
      boxpdb = "gcmc_box.pdb"
    else:
...
def _wizard(settings) :
  ...
  print "\t6) Grand-canonical Alchemical Potential, Single Topology (GCAP)"
  print "\t7) Grand-canonical Alchemical Potential, Dual Topology (GCAP)"
  ...
  if instr == "" : return
  val = int(instr)
  vals = ["equilibration","sampling","dualtopology","singletopology","gcmc","gcap_single","gcap_dual","jaws1","jaws2"]
  ...
  if settings.simulation not in ["dualtopology","singletopology","gcap_single","gcap_dual"] :
...
if __name__ == "__main__":
  ...
  parser.add_argument('-s','--simulation',choices=["none","equilibration","sampling","dualtopology","singletopology","gcmc","gcap_single","gcap_dual","jaws1","jaws2"],help="the kind of simulation to setup",default="none")
...
#    if args.simulation == "dualtopology" and args.absolute :
  ...
  if args.simulation in ["gcmc","gcap_single","gcap_dual","jaws1"] :
  ...
  for repeat in repeats :
    ...
    if not args.simulation in ["singletopology","gcap_single","jaws2"] or "_ele" in repeat :
    ...
    if bnd_cmd is not None :
      ...
      elif args.simulation in ["gcap_single","gcap_dual"] :
