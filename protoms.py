...
def _prep_gcmc(ligands,ligand_files,waters,tarlist,settings) :
  ...
  def arrange_wats(box,water_file,out_name) :
    ...
    if not (box_origen_below and box_end_avobe) or (np.all(waterbox['len'] < 0.5) and len(waterobj.solvents) > 1) :
      ...
      arranged_obj = tools.distribute_particles(box,water_file,ranseed=settings.ranseed)
  ...
  def fill_box (settings,boxpdb,box,ghost_name) :
    ...
    if settings.gcmcwater is None :
      ...
      ghostobj = tools.distribute_particles(box=box,particles=ghostnum,watermodel=settings.watmodel,ranseed=settings.ranseed)
...
#        for atom in ghostobj.solvents[sol].atoms : atom.resname = "WA1"
      ...
      ghostobj = tools.distribute_particles(box,settings.gcmcwater,watermodel=settings.watmodel,ranseed=settings.ranseed)
      ghostobj.write(filename=ghost_name)
    else :
...
if __name__ == "__main__":
  ...
  parser.add_argument('--ranseed',help="optional seed for random number generators in setup",default=None) 
