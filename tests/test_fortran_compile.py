...
class TestFortran(unittest.TestCase):
    ...
    def test_build_folder(self):
        ...
        if (
            call(
                "cd $PROTOMSHOME/build && cmake .. && make install || sudo make install",
                shell=True,
            )
            == 0
        ):
            print(
                "Fortran code is compiled correctly and ProtoMS is successfully built."
            )
        else:
            raise Exception(
                "Either build files have not been written successfully or make install failed."
            )
    ...
    def test_fortran(self):
        ...
        self.assertTrue(
            os.path.exists(os.environ["PROTOMSHOME"] + "/protoms3"),
            "protoms3 executable doesn't exist. Fortran code is not compiled properly.",
        )
