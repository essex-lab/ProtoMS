    ...
    def test_RETI_dbl(self):
        ...
        if((call("python2.7 $PROTOMSHOME/protoms.py -s dualtopology -l ethane.pdb methanol.pdb --nequil 0 --nprod 10 --lambdas 0.00 0.33 0.67 1.00 --ranseed 100000 --dumpfreq 1 --cleanup", shell=True)) == 0):
	        ...
	        try:
  		    ...
  		    print e
		    print "ProtoMS setup output file ",out_files, "is missing.", "There could be problems with ligand's zmat generation, forcefield issues, template generation issues and ProtoMS input command file generation for simulation."
                        ...
                        if len(dirs) != 0:
                            for d in dirs:
                                for out_files in out_sim_files:
                                    try:
                                        self.assertTrue(os.path.exists(os.path.join("out_free",d,out_files)))
                                    except IOError as e:
                                        print e
