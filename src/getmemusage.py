            ...
            if dim.isdigit():
                ...
                for dim in ds:
                    if dim.isdigit():
                        ...
                        if tmp is None:
                            tmp = str(parameters[dim.lower()].value)
                        else:
                            tmp = "%s + %s" % (
                                tmp,
                                str(parameters[dim.lower()].value),
                            )
                    else:
                        if tmp is None:
                            tmp = str(dim)
...
class Routine:
    ...
    def __init__(self, name, func=False, returns=None):
        ...
        if func:
            funcs[self.__name.lower()] = self
        else:
            subs[self.__name.lower()] = self
    ...
    while i < nlines:
        ...
        try:
            ...
            if words[0].lower() == "common":
                ...
                words[0].lower() == "integer"
                or words[0].lower() == "logical"
                or words[0].lower().find("character") == 0
                or words[0].lower() == "double"
...
if __name__ == "__main__":
    ...
    for f in files:
        if f.endswith(".inc"):
            # skip 'nb*.inc' files
            if f.find("nb") == 0 or f == "dimensions.inc":
                continue
    ...
    for v in variables:
        ...
        if sz >= 1:
            tmp = "%s\t%s(%s) %s (%s)" % (
                szstr,
                variable.name,
                variable.stringDimensions(),
                variable.type,
                variable.numberDimensions(),
            )
