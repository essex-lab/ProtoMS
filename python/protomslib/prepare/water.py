  ...
  """
    ...
    for sol in solvents:
        ...
        new_solvents[sol] = simulationobjects.Residue(
            name=watresname,
            index=sol)  # Creating a new pdb object that contains the water.
...
def rotatewat(watcoords, alpha, beta, gamma):
  ...
  """
    newmat = np.mat(watcoords - watcoords[0]).transpose(
    )  # Centering the oxgen at the origin.
    rotated = rotmat_x(alpha) * rotmat_y(beta) * rotmat_z(
        gamma) * newmat  # Rotating about the orgin
    return (rotated.transpose() + watcoords[0]
...
def rotatesolute(solutecoords, alpha, beta, gamma):
  ...
  """
    newmat = np.mat(solutecoords - np.mean(solutecoords, axis=0)).transpose(
    )  # Centering the oxgen at the origin.
    rotated = rotmat_x(alpha) * rotmat_y(beta) * rotmat_z(
        gamma) * newmat  # Rotating about the orgin
    return (rotated.transpose() + np.mean(solutecoords, axis=0)
            )  # Translating back to the location of the original oxygen.
...
def alignhydrogens(watcoords, template, tol=0.000001):
  ...
  Returns
    ...
    def MSD(angles):
        ...
        rw = np.array(rotatewat(template[0:3], angles[0],
                                angles[1], angles[2]))
...
def _rottranstemplate(solvents,
                      wattemplate,
                      watatomnames,
                      ignorH,
                      watresname=None):
    ...
    for sol in solvents:
        ...
        if len(solvents[sol].atoms) >= 3 and not bool(ignorH):
            ...
            wat = np.vstack(
                (solvents[sol].atoms[0].coords, solvents[sol].atoms[1].coords,
                 solvents[sol].atoms[2].coords))
            wat_new = alignhydrogens(wat, wat_new)
        else:  # ... if not, the template is randomly orientated.
            wat_new = rotatewat(wat_new, np.random.uniform(0, 2 * 3.142),
                                np.random.uniform(0, 2 * 3.142),
                                np.random.uniform(0, 2 * 3.142)).round(3)
        new_solvents[sol] = simulationobjects.Residue(
            name=wresnm,
            index=sol)  # Creating a new pdb object that contains the water.
        for ind in range(len(watatomnames)):
...
def convertwater(pdb_in, watermodel, ignorH=False, watresname=None):
    ...
    logger.debug("This will change the water molecule in the pdb "
                 "file to match the water model")

    pdb_out = pdb_in.copy()
    solvents = pdb_in.solvents
    # Ideal water geometries:
    t4p_model = np.array([[-6.444, -5.581, -1.154], [-7.257, -5.869, -0.739],
                          [-6.515, -4.627, -1.183], [-6.557, -5.495, -1.105]])
    t4p_names = ["O00", "H01", "H02", "M03"]
    t3p_model = np.array([[7.011, 5.276, 13.906], [6.313, 5.646, 13.365],
                          [6.666, 4.432, 14.197]])
    ...
    if watermodel.upper() in ["T4P", "TIP4P", "TP4"]:
        if watresname is None:
            ...
            wresnm = watresname
        pdb_out.solvents = _rottranstemplate(solvents, t4p_model, t4p_names,
                                             ignorH, wresnm)
        ...
        if watresname is None:
            ...
            wresnm = watresname
        pdb_out.solvents = _rottranstemplate(solvents, t3p_model, t3p_names,
                                             ignorH, wresnm)
    else:
        print("Error in convertwater.py: water model name not recognised."
  ...
  """
    for count, watobj in enumerate(water_files.pdbs):
        ...
        for i, coord in enumerate(
                watobj.residues[
                    list(watobj.residues.keys())[0]].atoms[0].coords):
