...
class ProtoMSSimulation:
    """
    This is a ProtoMS command file object.This object holds all of the
    information about the simulation and can either run the simulation with
    this information, or can write this information to a command file.
...
class DualTopology(ProteinLigandSimulation):
    ...
    """
        ...
        self.setParameter(
            "lambdare",
            "%d %s" % (2 * dumpfreq, " ".join("%.3f" % l for l in lambdaval)))
        ...
        if protein is not None:
            for restsol in restrained:
                ...
                for ind, tem in enumerate(templates):
                    temobj = simulationobjects.TemplateFile(
...
class RestraintRelease(ProteinLigandSimulation):
    """
    Command file to calculate the free energy of introducing a
...
class Jaws2(ProteinLigandSimulation):
    ...
    """
        ...
        for i, param in enumerate(["x", "y", "z"]):
            self.setParameter("origin" + param, "%.3f" % (ocoord[i] - 1.5))
        for i, param in enumerate(["x", "y", "z"]):
...
def generate_input(protein, ligands, templates, protein_water, ligand_water,
    ...
    if settings.simulation == "equilibration":
        ...
        bnd_cmd = GCMC(
            ...
            templates=templates,
            solvent=protein_water,
            gcmcwater=settings.gcmcwater,
            adamval=settings.adams,
            nequil=settings.nequil,
            gcmcbox=settings.gcmcbox,
            nprod=settings.nprod,
            dumpfreq=settings.dumpfreq,
            outfolder=outfolder,
            ranseed=ranseed,
        ...
        bnd_cmd = Jaws1(
            ...
            templates=templates,
            solvent=protein_water,
            jawswater=settings.gcmcwater,
            nequil=settings.nequil,
            jawsbox=settings.gcmcbox,
            ranseed=ranseed,
            nprod=settings.nprod,
            dumpfreq=settings.dumpfreq,
            outfolder=settings.outfolder)
