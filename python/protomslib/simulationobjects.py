...
class PDBFile:
    ...
    def copy(self):
        ...
        for res in self.residues:
            new.residues[res] = self.residues[res]
        for sol in self.solvents:
            new.solvents[sol] = self.solvents[sol]
        return new
        ...
        with open(filename, "r") as f:
            while True:
                ...
                if not (pdb.residues or pdb.solvents):
                    ...
                    if readmax is not None and len(self.pdbs) == readmax:
                        break
...
class SnapshotResults:
    ...
    def parse(self, fileobj):
        ...
        while line.find("RESULTS for lambda") == -1:
            line = fileobj.readline()
        cols = line.strip().split()
        if len(cols) == 5:
        ...
        while line[0] != "#":
            if line.startswith("Number of data steps"):
                self.datastep = int(line.split("=")[1].strip())
            if line.startswith("Lambda replica"):
                self.lambdareplica = int(line.split("=")[1].strip())
            if line.startswith("GCMC replica"):
                self.gcmcreplica = int(line.split("=")[1].strip().split()[0])
            if line.startswith("Temperature replica"):
                self.temperaturereplica = int(line.split("=")[1].strip())
            if line.startswith("Global replica"):
                self.globalreplica = int(line.split("=")[1].strip())
            ...
            if line.startswith("Simulation B factor"):
                self.bvalue = float(line.split("=")[1].strip())
            if line.startswith("Simulation B value"):
                self.bvalue = float(line.split("=")[1].strip())
            if line.startswith("Molecules in grid"):
                self.solventson = float(line.split("=")[1].strip())
            if line.startswith("Pressure"):
                self.pressure = float(line.split("=")[1].strip().split()[0])
            if line.startswith("Volume"):
                self.volume = float(line.split("=")[1].strip().split()[0])
            if line.startswith("Random number seed"):
                self.seed = int(line.split("=")[1].strip())
            if line.startswith("Backwards Free Energy"):
                self.backfe = float(line.split("=")[1].strip().split()[0])
            if line.startswith("Forwards  Free Energy"):
                self.forwfe = float(line.split("=")[1].strip().split()[0])
            line = fileobj.readline().strip()
...
class ResultsFile:
    ...
    def read(self, filename, skip=0, readmax=None):
        ...
        if isinstance(filename, six.string_types):
            ...
            nsnap = 0
            while True:
                while line and not line.startswith("RESULTS FILE"):
                    line = f.readline()
                if line:
                    nsnap += 1
                    if nsnap > skip:
                        ...
                        line = f.readline()
                    if readmax is not None and len(self.snapshots) == readmax:
                        break
                else:
                    break
            f.close()
...
class MolTemplate:
    ...
    def parse_from(self, fileobj):
        ...
        while line:
            ...
            elif line.startswith(("bond", "angle", "dihedral")):
    ...
    def write(self, filename):
        ...
        with open(filename, "w") as f:
            ...
            if self.dihedralparams:
                ...
                for term in self.dihedralterms:
                    f.write("%s\n" % term)
                f.write("#par  term1  term2 etc..  #comment\n")
                for par in self.dihedralparams:
                    f.write("%s\n" % par)
