...
amb_Parm.read_in(
    "/home/chris/Software/amber14/dat/leap/prep/oldff/all_amino94.in"
)
amb_Parm.read_in(
    "/home/chris/Software/amber14/dat/leap/prep/oldff/all_aminoct94.in",
    term="cterm",
)
amb_Parm.read_in(
    "/home/chris/Software/amber14/dat/leap/prep/oldff/all_aminont94.in",
    term="nterm",
)
amb_Parm.fix_diffs()
...
for i in prmtop.chunks(s.vals + s2.vals, 5):
    ...
    ids = map(prmtop.ptoi, map(abs, map(int, i[:4])))
...
with open("test/tmp_out") as f:
    for line in f:
        if line.startswith(" TAG"):
            ...
            reses = [
                i + "nt" if j == "1" else i for i, j in zip(reses, res_nos)
            ]
            reses = [
                i + "ct" if j == "157" else i for i, j in zip(reses, res_nos)
            ]
            ...
            at_types = [
                amb_Parm.cljs.cljbyres(i, j).type
                for i, j in zip(reses, amb_ats)
            ]
            dih = pms_parms.get_params(at_types)
            nterms = len(
                [i for i in dih.terms if i.k1 != 0.0]
            )  # get non-zero dihedral terms
